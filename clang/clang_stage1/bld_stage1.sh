...
conda-build -c local -m cbc_boot_from.yaml clangdev-feedstock
echo "First runtime library"
conda-build -c local -m cbc_boot_from.yaml libcxx-feedstock
conda-build -c local -m cbc_boot_from.yaml tapi-feedstock
...
echo "Hurry, we reached initial state ... now next stage"

...
conda-build -c local -e ../conda_build_config_4.yaml -m cbc_boot_from.yaml clang_bootstrap_osx-64-feedstock
echo "Now we can start to do standard ...dev builds for new version"
conda-build -c local -e ../conda_build_config_4.yaml -m cbc_boot_from.yaml llvmdev-feedstock
conda-build -c local -e ../conda_build_config_4.yaml -m cbc_boot_from2.yaml clangdev-feedstock
echo "First runtime library"
conda-build -c local -e ../conda_build_config_4.yaml -m cbc_boot_from2.yaml libcxx-feedstock
